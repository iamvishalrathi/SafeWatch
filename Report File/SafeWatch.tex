...
\centering
\textbf{SafeWatch System Architecture}
\vspace{0.5cm}

\framebox[0.9\textwidth][c]{
\begin{minipage}{0.85\textwidth}
\textbf{Frontend Layer}\\
React.js Interface $\rightarrow$ User Dashboard $\rightarrow$ Real-time Alerts\\
\vspace{0.2cm}

\textbf{API Gateway}\\
Flask RESTful API $\rightarrow$ Authentication $\rightarrow$ Route Management\\
\vspace{0.2cm}

\textbf{AI Processing Engine}\\
Gender Detection (CNN) $\rightarrow$ Gesture Recognition (MediaPipe) $\rightarrow$ Decision Fusion\\
\vspace{0.2cm}

\textbf{Data Layer}\\
SQLAlchemy ORM $\rightarrow$ SQLite Database $\rightarrow$ File Storage System
\end{minipage}
}
\end{minipage}
...
\begin{minipage}{0.8\textwidth}
\centering
\textbf{AI Processing Workflow}
\vspace{0.5cm}

\framebox[0.9\textwidth][c]{
\begin{minipage}{0.85\textwidth}
\textbf{Input Processing}\\
Video Stream $\rightarrow$ Frame Extraction $\rightarrow$ Face Detection (Haar Cascade)\\
\vspace{0.2cm}

\textbf{Gender Analysis}\\
Face ROI $\rightarrow$ CNN Model (Caffe) $\rightarrow$ Gender Classification (91.8\% accuracy)\\
\vspace{0.2cm}

\textbf{Gesture Recognition}\\
Hand Detection (MediaPipe) $\rightarrow$ Landmark Extraction $\rightarrow$ Gesture Classification (87.0\% accuracy)\\
\vspace{0.2cm}

\textbf{Decision Fusion}\\
Multi-frame Analysis $\rightarrow$ Confidence Scoring $\rightarrow$ Alert Generation
\end{minipage}
}
\end{minipage}
...
Stakeholder Interviews $\rightarrow$ Literature Review $\rightarrow$ Gap Analysis $\rightarrow$ System Requirements\\
\vspace{0.3cm}

$\downarrow$\\
\vspace{0.2cm}

\textbf{Phase 2: Design \& Development}\\
Architecture Design $\rightarrow$ AI Model Selection $\rightarrow$ Implementation $\rightarrow$ Integration Testing\\
\vspace{0.3cm}

$\downarrow$\\
\vspace{0.2cm}

\textbf{Phase 3: Validation \& Testing}\\
Performance Testing $\rightarrow$ User Evaluation $\rightarrow$ Security Assessment $\rightarrow$ Results Analysis\\
\vspace{0.3cm}

$\downarrow$\\
\vspace{0.2cm}

\textbf{Phase 4: Deployment \& Evaluation}\\
Pilot Deployment $\rightarrow$ Field Testing $\rightarrow$ Performance Monitoring $\rightarrow$ Final Validation
\end{minipage}
}
\end{minipage}
...
\begin{minipage}{0.8\textwidth}
\centering
\textbf{SafeWatch User Interface Layout}
\vspace{0.5cm}

\framebox[0.9\textwidth][c]{
\begin{minipage}{0.85\textwidth}
\textbf{Navigation Bar}\\
SafeWatch Logo | Dashboard | Live Feed | Alerts | Settings | Profile\\
\vspace{0.3cm}
...
\textbf{Status Bar}\\
System Status | Active Cameras: 6 | Processing Speed: 28 FPS | Alerts: 3
\end{minipage}
}
\end{minipage}
...
\begin{minipage}{0.8\textwidth}
\centering
\textbf{Performance Comparison Across Different Conditions}
\vspace{0.5cm}

\framebox[0.9\textwidth][c]{
\begin{minipage}{0.85\textwidth}
\textbf{Gender Detection Accuracy}\\
Daylight: 94.2\% | Artificial Light: 91.8\% | Low Light: 86.5\%\\
\vspace{0.3cm}

\textbf{Gesture Recognition Accuracy}\\
Clear View: 91.2\% | Partial Occlusion: 87.0\% | Complex Background: 82.4\%\\
\vspace{0.3cm}

\textbf{Real-time Performance (FPS)}\\
Single Camera: 30 FPS | 3 Cameras: 28 FPS | 6 Cameras: 25 FPS\\
\vspace{0.3cm}

\textbf{Response Time}\\
Alert Generation: <500ms | Database Update: <200ms | UI Refresh: <100ms
\end{minipage}
}
\end{minipage}
